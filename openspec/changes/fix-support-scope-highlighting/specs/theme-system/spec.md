...
#### Scenario: Support scope colors built-in elements

- **WHEN** the theme encounters a `support` TextMate scope (e.g., `support.type.exception`, `support.class`, `support.function`)
- **THEN** it SHALL apply `palette.type` color to make built-in types, classes, and functions visually distinct
- **AND** more specific rules (like `support.type.property-name`) MAY override this default
